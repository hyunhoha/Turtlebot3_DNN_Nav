...
model = tf.keras.models.load_model("/home/iasl/catkin_ws/src/knu_prj/res_collision_model/")
control_out = 0
...
def image_callback(image_data):
    ...
    a, b, c, d = -0.1, 0.5, 0.26, 0.5
    twist_data.linear.x = (control_out[0] * a) + (control_out[1] * 0.1) + (control_out[2] * c) + (control_out[3] * 0.1) - collision_prob[0] * 0.1
    if control_out[1] >= control_out[3]:
        twist_data.angular.z = control_out[1]*b * (1+collision_prob[0])
    else:
        twist_data.angular.z = -control_out[3]*d * (1+collision_prob[0])
