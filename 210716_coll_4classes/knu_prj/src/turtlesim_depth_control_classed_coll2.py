...
model = tf.keras.models.load_model("/home/iasl/catkin_ws/src/knu_prj/res_collision_model_new_5_classes/")
control_out = 0
...
def image_callback(image_data):
    ...
    a, b, c, d = -0.05, 0.5, 1, 0.5
    alpha = 0.8
    new_vel = (control_out[0] * a) + (control_out[1] * 0.1) + (control_out[2] * c) + (control_out[3] * 0.1) - collision_prob[0]*0.5
    ...
    if control_out[1] >= control_out[3]:
        twist_data.angular.z = alpha * (control_out[1]*b * (1+collision_prob[0])) + (1-alpha) * twist_data.angular.z
    else:
        twist_data.angular.z = (-control_out[3]*d * (1+collision_prob[0])) * alpha + (1-alpha) * twist_data.angular.z
