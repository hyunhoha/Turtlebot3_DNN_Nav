...
def image_callback(image_data):
    ...
    # process image and predict using deep-learning model.
    ...
    if control_out[1] >= control_out[3]:
        twist_data.angular.z = (0.01+ control_out[0] + collision_prob[0]) * alpha * 0.5 + (1-alpha) * twist_data.angular.z
    else:
        twist_data.angular.z = -(0.01 + control_out[0] + collision_prob[0]) * alpha * 0.5 + (1-alpha) * twist_data.angular.z
